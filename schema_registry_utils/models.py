...
A change in any HashSubset field produces a different sha256_hash (a different entity), never an edit of the old one; lineage is tracked via was_derived_from on each ProvenanceEntry.""", json_schema_extra = { "linkml_meta": {'domain_of': ['RegistryEntity']} })
    ...
    skos_mappings: Optional[list[Mapping]] = Field(default=None, description="""Semantic mappings to external vocabulary concepts, or to other registry entities (align.py's computed correspondences). Storage is embedded (the owning entity is the implicit subject); SSSOM export materializes subject_id from the owning entity's id, so exported mapping sets are standalone, standards-conformant records.""", json_schema_extra = { "linkml_meta": {'domain_of': ['RegistryEntity']} })
...
class SchemaSource(ConfiguredBaseModel):
    """
    Registry record for a schema source (one node per ingested schema label). Tracks the source's IRI, MIME type, and the registry version it was first added under. Identity uses id (as everywhere) because this is a mutable administrative record, not a content-addressed entity. Every attestation about a registry entity that names this schema as its primary source is surfaced on `attestations`, the inverse of ProvenanceEntry.had_primary_source — a schema-regen query can walk forward from a SchemaSource to every entity it defined in one hop.
