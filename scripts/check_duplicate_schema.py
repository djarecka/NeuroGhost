...
against every OTHER schema already committed under schemas/. If an identical
file exists, exit non-zero and name it — so the CI ingestion workflow fails
early with a clear message instead of silently ingesting a duplicate.

Usage:
    python scripts/check_duplicate_schema.py schemas/<name>.yml
"""
...
def find_duplicate(target: Path) -> Path | None:
    """Return an existing schema file with content identical to `target`, or None."""
    target = target.resolve()
    target_hash = content_hash(target.read_text())
    for other in sorted(SCHEMAS_DIR.glob("*.yml")):
