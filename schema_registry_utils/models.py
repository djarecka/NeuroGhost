...
class Rule(RegistryEntity):
    ...
    Identity is content-addressed like any RegistryEntity: same rule_type + same parameters + same targets + same defined_in_schema hash to the same Rule; a schema that changes only the human-readable `error_message` keeps its hash_id (see the HashSubset membership on each slot). Rules are attached to RegistryClass/RegistryProperty through the `applies_to` slot — not inlined on the entity itself, because the same property can carry different constraints in different sources' usage (a stance already made explicit in RegistryProperty's own description).
    """
    linkml_meta: ClassVar[LinkMLMeta] = LinkMLMeta({'from_schema': 'https://example.org/schema-registry-utils/meta-model'})
    ...
    applies_to: list[str] = Field(default=..., description="""The registry entities this rule constrains — typically one RegistryProperty for a property-level constraint (a pattern on `identifier`, a min_length on `description`), or a RegistryClass plus the RegistryProperty(ies) it references for a class-level cross-field constraint (dandi-schema's `@model_validator` on Contributor names Contributor plus `roleName` and `email`). Range is RegistryEntity because a rule may apply to a class, a property, or both; ingestion should only ever populate this with RegistryClass or RegistryProperty targets (PermissibleValue, ValueSet, Rule itself are not valid targets).""", json_schema_extra = { "linkml_meta": {'domain_of': ['Rule'], 'in_subset': ['HashSubset']} })
